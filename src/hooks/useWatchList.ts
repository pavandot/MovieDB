...
const useWatchList = (mediaType: string) => {
	const sessionId = useAppSelector((state) => state.auth.sessionId);
	// Fetcher function
	const getWatchList = async (mediaType: string, sessionId: string | null) => {
		const response = await axios.get(`https://api.themoviedb.org/3/account/%7Baccount_id%7D/watchlist/${mediaType}?api_key=${import.meta.env.VITE_REACT_APP_API_KEY}&session_id=${sessionId}&language=en-US&page=1`);

		let data: useWatchListReturn[] = [];
	...
	return useQuery(["watchlist", mediaType], () => getWatchList(mediaType, sessionId));
};
